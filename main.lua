...
local function AutoFarmLevel()
    ...
    elseif not queue:empty() and queue:top() == "Auto Farm Level" and getgenv().Weapon == nil then
        ...
        Auto_Farm_Level_Toggle:SetValue(false)
        getgenv().Auto_Farm_Level = false
    end
end
...
local function AutoFarmBone()
    if game.PlaceId ~= 7449423635 then
        ...
        Auto_Farm_Bone_Toggle:SetValue(false)
        getgenv().Auto_Farm_Bone = false
        return
    end
    local CFrame_Bone = {
    ...
    if not queue:empty() and queue:top() == "Auto Farm Bone" and getgenv().Weapon ~= nil then
        pcall(function()
            if getgenv().Auto_Claim_Quest_Farm_Bone then
                if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false or not string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Reborn Skeleton") or not string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Living Zombie") or not string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Demonic Soul") or not string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Posessed Mummy") then
                    local Level = game:GetService("Players").LocalPlayer.Data.Level.Value
...
local function AutoFarmKata()
    if game.PlaceId ~= 7449423635 then
        ...
        Auto_Farm_Kata_Toggle:SetValue(false)
        getgenv().Auto_Farm_Kata = false
        return
    end
    local CFrame_Kata = {
...
local function UpdateMobKataKilled()
    pcall(function()
        Mob_Kata_Killed:SetValue("Killed: " .. (500 - tonumber(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"):match("([-]?%d+)"))) .. "/500")
    end)
end
local function FPSBoost()
    pcall(function()
        ...
        local function CheckIfBad(Instance)
            if(Instance.Parent ~= game.Workspace.Map) then
                return
            end
...
AutoFarm:AddToggle({
	Title = "Auto Farm Boss Hop Server",
...
if not W3 then
    AutoFarm:AddToggle({
        Title = "Auto New World (Turn on with Auto Farm)",
    ...
    if W2 then
        ...
        local Select_Fruit = AutoFarm:AddDropdown({
            Title = "Select Fruit for Auto Third Sea",
            Default = nil,
            Values = Fruit_List,
            Callback = function(Value)
...
SubFarm:AddToggle({
	Title = "Auto Claim Quest When Farm Bone",
	Default = getgenv().Auto_Claim_Quest_Farm_Bone,
	Callback = function(Value)
		getgenv().Auto_Claim_Quest_Farm_Bone = Value
	end
})
Mob_Kata_Killed = SubFarm:AddParagraph("Killed: 0/500")
...
SubFarm:AddToggle({
	Title = "Auto Claim Quest When Farm Katakuri",
...
if W then
    Teleport:AddDropdown({
        ...
        Values = {"Wind Mill", "Town" , "Jungle" , "Pirate", "Mob boss", "Desert", "Marine Base", "Sky", "Prison", "Fountain", "Sky Area 2", "Whirl Pool", "Sky Area 1", "Magma", "Ice", "Marine Start", "Colosseum", "Fish Man"},
        Callback = function(Value)
            StopTween()
            tween(Islands[Value])
        end
...
elseif W2 then
    Teleport:AddDropdown({
        Title = "Teleport island",
        Default = "Choose island to teleport",
        Values = { "Dock" , "Swan Mansion" , "Kingdom Of Rose" , "Cafe" , "Colossuem" , "Factory" , "The Bridge" , "Green Bit" , "Graveyard" , "Dark Area" , "Snow Mountain" , "Hot Island" , "Cold Island" , "Ice Castle" , "Usopp's Island" , "Inscription Island" , "Forgotten Island" , "Ghost Ship" , "Mini Sky"},
        Callback = function(Value)
            StopTween()
            tween(Islands[Value])
        end
...
elseif W3 then
    Teleport:AddDropdown({
        Title = "Teleport island",
        Default = "Choose island to teleport",
        Values = { "Mini Sky" , "Port Town" , "Hydra Island" , "Giant Tree" , "Zou Island" , "Pineapple Village" , "Mansion" , "Castle on the sea" , "Haunted Castle" , "Ice Cream Island" , "Sea of treats" , "Cake Loaf", "Isle Outpost"},
        Callback = function(Value)
            StopTween()
            tween(Islands[Value])
        end
...
Teleport:AddSlider({
	Title = "Max players for hop low server",
	Min = 1,
	Max = 11,
...
-- Fruit
local Fruit = Window:AddTab({
    Title = "Fruit"
})
Fruit:AddDropdown({
	Title = "Select Fruit",
	Default = getgenv().Fruit_To_Snipe,
	Values = {"Kilo", "Spin", "Chop", "Spring", "Bomb", "Smoke", "Spike", "Flame", "Falcon", "Ice", "Sand", "Dark", "Revive", "Diamond", "Light", "Rubber", "Barrier", "Magma", "Quake", "Buddha", "Love", "Spider", "Phoenix", "Portal", "Rumble", "Paw", "Blizzard", "Gravity", "Dough", "Shadow", "Venom", "Control", "Spirit", "Dragon", "Leopard", "Kitsune"},
	Callback = function(Value)
...
Fruit:AddToggle({
	Title = "Auto Bring Fruit Hop Server",
...
Shop:AddDropdown({
	Title = "Sword",
	Default = nil,
	Values = {"Cutlass ($1.000)", "Dual Katana ($12.000)", "Katana ($1.000)", "Iron Mace ($25.000)", "Triple Katana ($60.000)", "Dual-Headed Blade ($400.000)", "Pipe ($100.000)", "Soul Cane ($750.000)", "Bisento ($1.000.000)"},
	Callback = function(Value)
...
Shop:AddDropdown({
	Title = "Gun",
	Default = nil,
	Values = {"Slingshot ($5.000)", "Flintlock ($10.500)", "Musket ($8.000)", "Cannon ($100.000)", "Refined Flintlock ($65.000)", "Refined Slingshot ($30.000)", "Kabucha (1.500F)"},
	Callback = function(Value)
        local item = string.sub(Value, 1, string.find(Value, "%(") - 2):gsub("^%s*(.-)%s*$", "%1")
		game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyItem",item)
	end    
...
Shop:AddDropdown({
	Title = "Melee",
	Default = nil,
	Values = {"Black Leg ($150.000)", "Electro ($500.000)", "Fishman Karate ($750.000)", "Dragon Breath (1500F)", "Superhuman ($3.000.000)", "Death Step ($2.500.000 & 5.000F)", "Electric Claw ($3.000.000 & 5.000F)", "Sharkman Karate ($2.500.000 & 5.000F)", "Dragon Talon ($3.000.000 & 5.000F)", "Godhuman ($5.000.000 & 5.000F)"},
	Callback = function(Value)
    ...
    while wait(0.01) do
        ...
        if getgenv().Auto_Farm_Boss then
            if not getgenv().Weapon then
                ...
                Auto_Farm_Boss_Toggle:SetValue(false)
                getgenv().Auto_Farm_Boss = false
                Check_Boss = false
