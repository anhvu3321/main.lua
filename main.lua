        ...
        if game:GetService("Players").LocalPlayer.PlayerGui.Main.Timer.Visible == false then
            if not Dungeon_Wait then
                ...
                if not game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Special Microchip") and not game:GetService("Players").LocalPlayer.Character:FindFirstChild("Special Microchip") then
                    ...
                    OrionLib:MakeNotification({
                        Name = "Error",
                        Content = "Fruit below 1M found: " .. fruit,
                        Image = "rbxassetid://4483345998",
                        Time = 3
                    })
                    if fruit == nil then
                        if getgenv().Auto_Raid_Hop then
                            OrionLib:MakeNotification({
                                Name = "Status",
                                Content = "Uh-oh. Seems like we have to collect some fruits.",
                                Image = "rbxassetid://4483345998",
                                Time = 3
                            })
                            if CheckFruit() then
                                OrionLib:MakeNotification({
                                    Name = "Status",
                                    Content = "Oh, fruits found! Gotta get it!",
                                    Image = "rbxassetid://4483345998",
                                    Time = 1
                                })
                                for i, v in pairs(game:GetService("Workspace"):GetChildren()) do
...
spawn(function()
    while task.wait(0.01) do
        pcall(function()
            AddStats()
            SafeAutoFarmLevel()
            if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("ColorsDealer", "1") ~= nil then
