...
local function AutoFarmBoss(boss)
    ...
    if not CFrameBoss then
        for _, v in pairs(game:GetService("ReplicatedStorage"):GetChildren()) do
            if v.Name:find(boss, 1, true) then
...
elseif game.PlaceId == 7449423635 then
    ...
    Bosses = {"rip_indra", "Soul Reaper", "Cake Prince", "Dough King"}
end
AntiReset()
-- UI
    ...
    while wait() do
        ...
        if getgenv().Auto_Farm_Boss then
            ...
            elseif getgenv().Boss_To_Farm ~= nil then
                ...
                if not CFrameBoss then
                    pcall(function()
                        CFrameBoss = CFrame.new(game:GetService("ReplicatedStorage"):FindFirstChild(getgenv().Boss_To_Farm).WorldPivot.Position) * CFrame.new(0, 30, 0)
                    end)
                end
                if not CFrameBoss then
                    pcall(function()
                        CFrameBoss = game:GetService("Workspace").Enemies:FindFirstChild(getgenv().Boss_To_Farm):FindFirstChild("HumanoidRootPart").CFrame * CFrame.new(0, 30, 0)
                    end)
                end
                if CFrameBoss ~= nil then
